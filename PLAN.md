...
|---|---|---|
| [`johnlombardo-dev/agent-mail`](https://github.com/johnlombardo-dev/agent-mail) | Non-empty repository with default branch `main`; this implementation has first-party history and the correct `origin`. | Canonical delivery target; verify exact branch, commit, PR, and CI state at each delivery. |
...
Repository initialization, canonical remote attachment, and first-party history are complete. The active implementation is a dirty worktree, so evidence and delivery claims must name an exact commit plus relevant diff fingerprint until the candidate is committed.
...
Use the existing Hermes slug `agent-mail` and permanent range `6110–6119`; do not delete or reallocate it. Rechecked 2026-08-20, the registry display name is “Agent Mail” and its base path is `/Volumes/Jove/Developer/Projects/agent-mail`.
...
| F09-R | Historical F09 bundled public reachability with a persisted undispatched attempt recovered after plan expiry; creation-time expiry did not protect recovery dispatch. | After authority expiry, no new remote effect begins; only an already-dispatched uncertain attempt may reconcile read-only. | Revalidate plan state, claim/version, target identity, and expiry immediately before every new effect, including recovered attempts and later targets; classify expired undispatched attempts durably. | Persist an undispatched attempt before expiry, recover after expiry, and assert zero precondition reads that mutate, zero mutation-adapter calls, and a durable non-executing result; repeat between targets. | #202, #183, #185 | Stage 2, before action CLI/live mutation; Stage 4 reruns the original reproduction and adjacent counterexample. | commit-bound local pass at `df02745`; current candidate load-blocked because `actionResultReconciliationSequence` is not exported; Stage 4 release-blocking |
