...
Status: design candidate `1.0.0-candidate.6`. Issue #196 freezes the required public `(incarnationId, version)` identity, six fixed-message sync-control errors, and the constructive 85-cell resolver. Candidate.6 preserves candidate.5 and dispositions R189-01 through R189-17 as resolved in the oracle; independent exact-digest review remains required.
...
Each cleanup state declares `minimumScope`. The resource registry owns one cleanup session and all idempotent release slots. Except for the exact frozen P3-C07 queue subtree, before an actor or descendant acquires or starts any external resource that could survive exit, it synchronously registers one direct slot keyed by owner scope and stable resource identity. Registration creates the one awaited terminal immediately. The resource becomes live only after registration completes; acquisition failure settles the existing slot.
...
The composite release trigger calls exact `queue.stop()` once, and the registry slot terminal aliases/adopts that exact promise. Its audit identity is the one stable parent slot ID: it appears once in workflow scope and `frozenReleaseSetId`, and contributes one live/unresolved unit until the stop promise settles. This is not a parallel close wrapper.
