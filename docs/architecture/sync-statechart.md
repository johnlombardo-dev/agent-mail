...
Status: design candidate `1.0.0-candidate.8`. Issue #196 freezes the required public `(incarnationId, version)` identity, six fixed-message sync-control errors, and the constructive 85-cell resolver. Candidate.8 preserves every R189-01 through R189-19 disposition and closes R189-20 with one exact permanent/invariant cleanup-terminal fault authority plus a total actor-boundary normalizer. R189-01 through R189-20 are resolved in the oracle; independent exact-digest review remains required.
...
The 24 typed machine events are: `xstate.init`; four `control.*.requested` events; `lifecycle.restart.requested`; `process.shutdown.requested`; `credentials.changed`; done/error events for `bootstrapSession`, `initialBackfill`, `recurringSweep`, and `cleanupBarrier`; four IDLE events; two watch-timer events; and two retry-timer events. Authentication errors carry the attempted credential revision. Cleanup success is `output:CleanupCertificate`; cleanup error is `error:CleanupTerminalFault` with required `error.releaseCertificate:CleanupCertificate`. The external send adapter admits exactly the four controls, restart, shutdown, and credential change. It exposes no constructor for `xstate.done.actor.*`, `xstate.error.actor.*`, or any actor-origin event; all 17 internal/reserved event types reject before `actor.send`.

`CleanupCertificate` is the one sealed cleanup-certificate authority. Its 13 exact leaf fields are certificate ID, cleanup epoch, cleanup phase, invoke lease, effective scope, frozen release-set ID, literal `released:true`, audit scope, audit frozen release-set ID, literal zero live-resource count, literal zero unresolved-release count, audit digest, and bounded safe diagnostics. `cleanupBarrier`, `CleanupTerminalFault.releaseCertificate`, `WorkflowFault.releaseCertificate`, both terminal events, both cleanup guards, `appendCleanupDiagnostics`, GEP05, and cleanup admission reference this type rather than copying a partial shape. The structural closure check resolves `CleanupCertificate.*` and requires the same 13 fields in both terminal contracts.

`CleanupTerminalFault` is a sealed exact four-field refinement of `WorkflowFault`: required `category`, `code`, `safeMessage`, and `releaseCertificate`; no `authReason` or extra field. Its discriminant is the literal union `permanent | invariant`, exactly the set accepted by `faultIsFatal` and all four cleanup-error transition definitions. `code` and `safeMessage` must pass the accepted safe code/message schemas, and `releaseCertificate` is the exact type above. This type is internal to the actor terminal and adds no public error code or response field.
...
| `cleanupBarrier`      | `fromPromise`                 | every closing/pausing/stopping state | One bounded phase/invoke adapter; error is exactly permanent/invariant `CleanupTerminalFault`; registry is the sole release owner         |
...
A pre-promotion watch terminal has the old phase and cannot release a workflow-required state. If watch settles and queues its done event before pause wins mailbox processing, promotion creates a fresh workflow phase from the immutable source-tree snapshot; the old invoke event is ignored and the replacement settles only from the new workflow audit. If watch done wins first, `T140` finishes and purges that session, and the later pause creates a new workflow epoch. If an equal-scope redirect replaces an already-settled wrapper, the registry resolves a new per-invoke promise from the cached current-phase terminal and binds the new lease. Current-pending, current-cached, promoted, and equal-scope certificates are snapshot-owned and therefore byte-identical with or without retired slot entries. No path waits for an impossible replay or second terminal from an old promise. Error settlement requires the same exact `CleanupCertificate` plus a permanent/invariant `CleanupTerminalFault`. `finishCleanupScope` alone closes and purges the registry session.

The TypeScript implementation boundary is structural, not a prose convention. The `cleanupBarrier` `fromPromise` body catches every unknown synchronous or asynchronous failure and never rethrows a broad `WorkflowFault` or unknown value. It waits for the registry's certified zero-audit settlement and strict-parses the complete terminal. A certified permanent/invariant failure is retained. Authentication, transient, or unknown internal material is redacted and converted by a total non-throwing normalizer to category `invariant`, code `sync.cleanup-terminal-contract`, and safe message `Cleanup terminal violated its fault-category contract.`, with the exact certified release certificate attached. Original messages, causes, provider material, credentials, and `authReason` are discarded; only already-redacted bounded certificate diagnostics survive. A malformed or invalid certificate stays inside the adapter: it keeps or resumes the authoritative registry wait and may reject later only after valid certification. If no certificate exists, the adapter remains pending instead of rejecting.

The four cleanup-error definitions expand exactly once over all 13 cleanup states. A current first permanent or invariant rejection with a valid certificate selects one local route: 12 states reach `stopped.failed`; `stopping.forShutdown` reaches `stopped.shutdown`; both record the accepted `sync.terminal-failure` diagnostic. Installed XState 5.32.5 probes require the root snapshot to remain `active` or become `done`, never `error`. GEP03 remains solely the actual exited-invoke/duplicate-identity rule. GEP05 is a pure model-selector rule for invalid current certificate fixtures: it selects no modeled transition/action, while the conforming actor boundary prevents that material from becoming an actual rejection. Reserved terminal types cannot be sent externally. No claim is made that XState ignores an invalid rejection after those boundaries are bypassed; bypass is a conformance violation.
