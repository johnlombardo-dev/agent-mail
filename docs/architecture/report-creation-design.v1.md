...
This JSON file is the single signed and normative authority for report creation, normalized-text materialization, persistence, source serving, and CLI behavior. It freezes user-selected capacity Policy A and consumes the accepted #233 canonical migration registry plus the #234 implementation at the accepted head. The design, decisions, and coverage Markdown files are exact checked projections of this authority.
...
Migration: Create report-creation-v1 only at packages/storage/src/migrations/0028-report-creation.ts and append that exact semantic migration as canonical registry slot 28 after the accepted immutable 1..27 prefix whose identity is 39971e45e0fe51580b0343d05b935a7583e42544b2f96ba6468bd813a11b68ab. Preserve accepted historical conversion target 27: the protected converter must execute and ledger only the 1..27 target slice and return at user_version 27, after which the common opener runs slot 28 only through applyMigrations and the inside-BEGIN verifyCanonicalMigrationPrefixState hook before any suffix effect. Update the registry-derived live full digest, schema and next-slot constants, registry test, and focused conversion composition test; do not change the protected converter or #233 artifacts, renumber or edit a predecessor, substitute the live full digest into historical provenance, loop the converter over the live registry, move the suffix runner behind a non-legacy branch, or bypass opener, doctor, backup, or restore authority.

Prefix evolution: Append slot 28 only through the accepted canonical registry. The accepted real legacy converter resolves explicit target 27, executes and ledgers only canonicalDatabaseMigrations.slice(0, 27), sets user_version 27, verifies that exact base, and returns without observing the live suffix. The common production opener then always invokes applyMigrations over the complete live registry; inside the slot-28 BEGIN IMMEDIATE transaction, beforePendingMigration verifies exact prefix 27 before any report SQL, schema_migrations INSERT, user_version write, or commit. Preserve every 1..27 semantic byte and immutable target-27 provenance byte; the new live full-registry digest must differ from and never replace target_registry_sha256. The converter source remains byte-protected; update only the focused conversion composition test to assert the exact historical prefix-27 digest, exact live registry-28 digest, real legacy-to-27-to-gated-28 sequence, append stability, and full-digest-substitution rejection.
