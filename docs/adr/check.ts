...
for (const match of index.matchAll(/^\| (ADR-\d+) \|.*\| (Accepted|Deferred|Evidence-only|Superseded) \|/gm)) {
...
for (const file of files) {
  ...
  if (!id || !status || !owner || !decision) errors.push(`${file}: missing ADR ID/status/owner/decision`);
  if (id && file !== `${id}.md`) errors.push(`${file}: filename does not match ${id}`);
  if (id && !indexed.has(id)) errors.push(`${file}: ADR is not indexed`);
  if (id && indexed.get(id) !== status) errors.push(`${file}: index status ${indexed.get(id) ?? "missing"} disagrees with record status ${status ?? "missing"}`);
  if (id) {
  ...
  if (status === "Accepted" && decision) {
    ...
    if (acceptedDecisions.has(key)) errors.push(`${file}: duplicate Accepted decision also in ${acceptedDecisions.get(key)}`);
    acceptedDecisions.set(key, file);
  }
  for (const line of text.split("\n")) {
    if (/(?:agent-mail-(?:sol|sol-luna)|agent-mail-proto|\/Volumes\/Jove\/Developer\/Projects\/agent-mail-sol-luna)/.test(line) && !/evidence-only/i.test(line)) {
      errors.push(`${file}: prototype path/identity presented without evidence-only label`);
    }
  }
}
for (const [id] of indexed) if (!ids.has(id)) errors.push(`README.md: indexed ADR ${id} has no record`);
if (!ids.has("ADR-001")) errors.push("missing ADR-001");
if (errors.length) {
  console.error(errors.map((error) => `FAIL ${error}`).join("\n"));
  process.exit(1);
}
console.log(`PASS ${files.length} markdown files; ${ids.size} unique ADR IDs; links, statuses, and evidence labels valid`);
