...
describe("canonical migration registry", () => {
  test("is the immutable contiguous application authority", () => {
    ...
    expect(CANONICAL_DATABASE_SCHEMA_VERSION).toBe(27);
    expect(NEXT_DATABASE_MIGRATION_VERSION).toBe(28);
    expect(NEXT_REPORT_MIGRATION_VERSION).toBe(28);
    ...
    expect(canonicalDatabaseMigrations.map(migrationContentHash)).toHaveLength(27);
    database.close();
  });
});
